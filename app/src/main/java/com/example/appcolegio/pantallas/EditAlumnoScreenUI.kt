...
fun EditarAlumno(onBack: () -> Unit, codigo: Int) {
    ...
    var datos by remember { mutableStateOf<Alumno?>(null) }
    LaunchedEffect(true) {
        datos = RetrofitCliente.alumnoApi.buscarPorCodigo(codigo).data
        datos?.let {
            nombre = it.nombre
            paterno = it.paterno
            materno = it.materno
...
//            )
            ...
            Column(
                ...
                } else if (!datos?.foto.isNullOrEmpty()) {
                    AsyncImage(
                        model = datos?.foto,
                        contentDescription = null,
                        modifier = Modifier.size(250.dp)
                    ...
                    onClick = {
                        scope.launch {
                            try {
                                ...
                                RetrofitCliente.alumnoApi.actualizarAlumnos(
                                    Alumno(
                                        codigo = datos!!.codigo,
                                        nombre = nombre,
                                        paterno = paterno,
                                ...
                                snackbar.showSnackbar("Alumno actualizado")
